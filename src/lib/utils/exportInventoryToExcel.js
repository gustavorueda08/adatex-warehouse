...
export async function exportInventoryToExcel({ filters, columns, toast }) {
  ...
  try {
    ...
    titleRow.getCell(1).font = { name: 'Arial', size: 16, bold: true, color: { argb: 'FFFFFFFF' } }; // White
    ...
    dateRow.getCell(1).font = { name: 'Arial', size: 10, italic: true, color: { argb: 'FFD4D4D8' } }; // Zinc 300
      ...
      products.forEach(product => {
        ...
        columns.forEach(col => {
            if (col.getValue) {
                ...
                rowData[col.key] = product[col.key] || product.attributes?.[col.key] || "";
            }
        });
