...
export const AVAILABLE_ITEM_COLUMNS = [
    ...
    { header: "Fecha Creación", key: "createdAt", width: 20, label: "Fecha Creación" },
 ...
 */
export async function exportItemsToExcel({ filters, columns = AVAILABLE_ITEM_COLUMNS, toast }) {
  let loadingToast;
  try {
    ...
    // 1. Fetch Data (Pagination loop over PRODUCTS)
    const allRows = [];
    const pageSize = 100;
    let page = 1;
    let pageCount = 1;
    ...
    do {
      // Use exact same params as Inventory Export, but add includeItems
      const params = {
        filters: filters || {},
        pagination: { page, pageSize },
        populate: ['collections'], 
        sort: ['name:asc'],
        includeItems: true, // Custom backend param
      };

      const queryString = buildStrapiQuery(params);
      // Use the inventory endpoint which we know works for products context
      const res = await fetch(`/api/strapi/products/inventory?${queryString}`);
      ...
      products.forEach(product => {
        ...
        items.forEach(item => {
            ...
            if (warehouse && warehouse.name) {
                // Ensure all possible keys are available for selection
                allRows.push({
                    id: item.id || "-",
                    code: product.code || "-",
                    barcode: item.barcode || item.attributes?.barcode || "-", 
                    name: product.name || "-",
                    state: stateMap[item.status || item.state] || item.status || item.state || "Disponible", 
                    unit: product.unit || "-",
    ...
    dateRow.getCell(1).font = { name: 'Arial', size: 10, italic: true, color: { argb: 'FFD4D4D8' } }; 
    ...
    a.href = url;
    a.download = `Adatex-Detalle-Items-${new Date().toISOString().split('T')[0]}.xlsx`;
    a.click();
