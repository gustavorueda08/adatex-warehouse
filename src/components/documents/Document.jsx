...
function DocumentHeaderField({ field = {} }) {
...
function DocumentHeader(props) {
  const { document, title, children } = props;
  const screenSize = useScreenSize();
  const documentCodeTitle = () => {
    let title = "";
    if (document?.code) {
      title += document.code;
    }
    if (document?.containerCode) {
      title += ` | ${document.containerCode}`;
    }
    if (document?.invoiceNumberTypeA) {
      title += ` | ADTX-${document.invoiceNumberTypeA}`;
    }
    if (document?.invoiceNumberTypeB) {
      title += ` | AD-${document.invoiceNumberTypeB}`;
    }
    return title;
  };
  return (
    <Card>
      <CardHeader className="gap-2 pb-0 md:pb-3 md:pt-4 flex flex-col items-start">
        <div className="flex items-center gap-2">
          ...
          {document?.state && (
            <Chip color={getChipVariant(document)} size={screenSize}>
...
export default function Document(props) {
  ...
  return (
    <div className="space-y-6 pb-8 w-full flex flex-col">
      <DocumentHeader document={document} title={title}>
