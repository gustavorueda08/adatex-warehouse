...
export async function PUT(request, context) {
  try {
    ...
    // Obtener el ID del territorio
    const { territoryId } = await context.params;

    if (!territoryId) {
      return NextResponse.json(
        { error: "ID del territorio requerido" },
        { status: 400 },
      );
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: "Error al actualizar el territorio en Strapi",
          details: errorText,
          status: response.status,
        },
        { status: response.status },
...
export async function DELETE(request, context) {
  try {
    ...
    console.log(
      `🗑️ Attempting to DELETE territory ${territoryId} from:`,
      strapiUrl.toString(),
    );
    ...
    // Verificar si el territorio realmente se eliminó
    if (response.ok) {
      // Intentar hacer un GET para verificar si el territorio aún existe
      setTimeout(async () => {
        try {
          ...
          if (verifyResponse.ok) {
            console.log(
              `⚠️ WARNING: Territory ${territoryId} STILL EXISTS after DELETE!`,
            );
          } else if (verifyResponse.status === 404) {
            console.log(
              `✅ VERIFIED: Territory ${territoryId} was successfully deleted`,
            );
          } else {
            console.log(
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: "Error al eliminar el territorio en Strapi",
          details: errorText,
          status: response.status,
        },
        { status: response.status },
    ...
    if (
      ...
      console.log(
        `✅ Territory ${territoryId} deleted successfully (204 No Content)`,
      );
      return NextResponse.json(
        {
          data: {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      if (text.trim().length === 0) {
        console.log(
          `✅ Territory ${territoryId} deleted successfully (empty JSON)`,
        );
        return NextResponse.json(
          {
            data: {
      ...
      console.log(
        `✅ Territory ${territoryId} deleted, Strapi response:`,
        data,
      );
    ...
    console.log(
      `✅ Territory ${territoryId} deleted successfully (non-JSON response)`,
    );
    return NextResponse.json(
      {
        data: {
