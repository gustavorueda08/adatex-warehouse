import { getTokenFromCookies } from "@/lib/auth/session";
import { NextResponse } from "next/server";

const STRAPI_URL = process.env.STRAPI_URL;

/**
 * PUT - Actualizar una orden específica
 */
export async function PUT(request, context) {
  try {
    const token = await getTokenFromCookies();
    if (!token) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    // Obtener el ID de la colección
    const { collectionId } = await context.params;

    if (!collectionId) {
      return NextResponse.json(
        { error: "ID de la colección requerido" },
        { status: 400 },
      );
    }

    // Obtener el body de la petición
    const body = await request.json();

    if (!body || !body.data) {
      return NextResponse.json(
        {
          error: "Datos del customer requeridos. Se requiere un objeto 'data'",
        },
        { status: 400 },
      );
    }

    // Construir la URL de Strapi
    const strapiUrl = new URL(
      `/api/collections/${collectionId}`,
      STRAPI_URL.toString(),
    );

    // Configurar headers
    const headers = {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    };

    // Realizar la petición PUT a Strapi
    const response = await fetch(strapiUrl.toString(), {
      method: "PUT",
      headers,
      body: JSON.stringify(body),
    });

    if (!response.ok) {
      const errorText = await response.text();
      console.error("Strapi Error:", {
        status: response.status,
        statusText: response.statusText,
        error: errorText,
        url: strapiUrl.toString(),
      });

      return NextResponse.json(
        {
          error: "Error al actualizar la colección en Strapi",
          details: errorText,
          status: response.status,
        },
        { status: response.status },
      );
    }

    const data = await response.json();

    // Validar estructura de respuesta
    if (!data || typeof data !== "object") {
      return NextResponse.json(
        { error: "Respuesta inválida de Strapi" },
        { status: 500 },
      );
    }

    return NextResponse.json(data, { status: 200 });
  } catch (error) {
    console.error("API Route Error:", error);
    return NextResponse.json(
      {
        error: "Error interno del servidor",
        message: error.message,
      },
      { status: 500 },
    );
  }
}

/**
 * DELETE - Eliminar una orden específica
 */
export async function DELETE(request, context) {
  try {
    const token = await getTokenFromCookies();
    if (!token) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    // Obtener el ID de la colección
    const { collectionId } = await context.params;

    if (!collectionId) {
      return NextResponse.json(
        { error: "ID de la colección requerido" },
        { status: 400 },
      );
    }

    // Construir la URL de Strapi
    const strapiUrl = new URL(
      `/api/collections/${collectionId}`,
      STRAPI_URL.toString(),
    );

    console.log(
      `🗑️ Attempting to DELETE collection ${collectionId} from:`,
      strapiUrl.toString(),
    );

    // Configurar headers
    const headers = {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    };

    // Realizar la petición DELETE a Strapi
    const response = await fetch(strapiUrl.toString(), {
      method: "DELETE",
      headers,
    });

    console.log(`📬 DELETE Response:`, {
      status: response.status,
      statusText: response.statusText,
      ok: response.ok,
      contentType: response.headers.get("content-type"),
      contentLength: response.headers.get("content-length"),
    });

    // Verificar si la colección realmente se eliminó
    if (response.ok) {
      // Intentar hacer un GET para verificar si la colección aún existe
      setTimeout(async () => {
        try {
          const verifyResponse = await fetch(strapiUrl.toString(), {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });

          if (verifyResponse.ok) {
            console.log(
              `⚠️ WARNING: Collection ${collectionId} STILL EXISTS after DELETE!`,
            );
          } else if (verifyResponse.status === 404) {
            console.log(
              `✅ VERIFIED: Collection ${collectionId} was successfully deleted`,
            );
          } else {
            console.log(
              `❓ Unable to verify deletion, status: ${verifyResponse.status}`,
            );
          }
        } catch (err) {
          console.log(`❌ Error verifying deletion:`, err.message);
        }
      }, 500);
    }

    if (!response.ok) {
      const errorText = await response.text();
      console.error("Strapi Error:", {
        status: response.status,
        statusText: response.statusText,
        error: errorText,
        url: strapiUrl.toString(),
      });

      return NextResponse.json(
        {
          error: "Error al eliminar la colección en Strapi",
          details: errorText,
          status: response.status,
        },
        { status: response.status },
      );
    }
    // Manejar respuesta vacía (HTTP 204 o sin contenido)
    if (
      response.status === 204 ||
      response.headers.get("content-length") === "0"
    ) {
      console.log(
        `✅ Collection ${collectionId} deleted successfully (204 No Content)`,
      );
      return NextResponse.json(
        {
          data: {
            id: parseInt(collectionId),
            deletedAt: new Date().toISOString(),
          },
        },
        { status: 200 },
      );
    }

    // Intentar parsear JSON solo si hay contenido
    const contentType = response.headers.get("content-type");
    if (contentType && contentType.includes("application/json")) {
      const text = await response.text();
      if (text.trim().length === 0) {
        console.log(
          `✅ Collection ${collectionId} deleted successfully (empty JSON)`,
        );
        return NextResponse.json(
          {
            data: {
              id: parseInt(collectionId),
              deletedAt: new Date().toISOString(),
            },
          },
          { status: 200 },
        );
      }

      const data = JSON.parse(text);
      console.log(
        `✅ Collection ${collectionId} deleted, Strapi response:`,
        data,
      );

      // Validar estructura de respuesta
      if (!data || typeof data !== "object") {
        return NextResponse.json(
          { error: "Respuesta inválida de Strapi" },
          { status: 500 },
        );
      }

      return NextResponse.json(data, { status: 200 });
    }

    // Si no es JSON, asumir éxito
    console.log(
      `✅ Collection ${collectionId} deleted successfully (non-JSON response)`,
    );
    return NextResponse.json(
      {
        data: {
          id: parseInt(collectionId),
          deletedAt: new Date().toISOString(),
        },
      },
      { status: 200 },
    );
  } catch (error) {
    console.error("API Route Error:", error);
    return NextResponse.json(
      {
        error: "Error interno del servidor",
        message: error.message,
      },
      { status: 500 },
    );
  }
}
