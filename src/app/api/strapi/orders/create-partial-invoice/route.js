...
/**
 * POST - Crear una factura parcial con búsqueda automática FIFO
 */
  ...
  try {
    ...
    if (!body) {
      return NextResponse.json(
        { error: "Datos requeridos para crear la factura parcial" },
        { status: 400 }
      );
    }

    // Validar campos requeridos
    if (!body.parentOrder || !body.customer || !body.products) {
      return NextResponse.json(
        {
          error:
            "Los campos 'parentOrder', 'customer' y 'products' son requeridos",
        },
        { status: 400 }
      );
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: "Error al crear la factura parcial en Strapi",
          details: errorText,
          status: response.status,
        },
        { status: response.status }
